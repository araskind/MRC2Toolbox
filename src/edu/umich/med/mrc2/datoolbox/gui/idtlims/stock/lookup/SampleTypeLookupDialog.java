...
public class SampleTypeLookupDialog extends JDialog implements ActionListener{
	...
	public SampleTypeLookupDialog(ActionListener listener) {
		super();
		setPreferredSize(new Dimension(500, 350));
		setSize(new Dimension(500, 350));
		setTitle("Lookup sample type in database");
		...
		searchButton.setActionCommand(MainActionCommands.SEARCH_SAMPLE_TYPES_COMMAND.getName());
		...
		btnSelect = new JButton(MainActionCommands.SELECT_SAMPLE_TYPE_COMMAND.getName());
		btnSelect.setActionCommand(MainActionCommands.SELECT_SAMPLE_TYPE_COMMAND.getName());
		...
		loadExistingSampleTypes();
		pack();
	}
	
	private void loadExistingSampleTypes() {
		
		Collection<LIMSSampleType>existingSampleTypes = new TreeSet<LIMSSampleType>();
		try {
			existingSampleTypes = IDTUtils.getAvailableSampleTypes();
		} catch (Exception e) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getActionCommand().equals(MainActionCommands.SEARCH_SAMPLE_TYPES_COMMAND.getName())) {
			searchSampleTypeDatabase();
		}
	}
	...
	private void searchSampleTypeDatabase() {
		...
		SampleTypeLookupTask task = new SampleTypeLookupTask();
		idp = new IndeterminateProgressDialog("Searching sample types database ...", this.getContentPane(), task);
	...
	class SampleTypeLookupTask extends LongUpdateTask {
		...
		public Void doInBackground() {
			...
			try {
				sampleTypes = IDTUtils.lookupSampleTypesInDatabase(textField.getText().trim());
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
