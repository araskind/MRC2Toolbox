...
public class DockableInputFilesPanel extends DefaultSingleCDockable 
	...
	public void actionPerformed(ActionEvent event) {
		...
		if(command.equals(MainActionCommands.OPEN_POSITIVE_MODE_RAW_DATA_FOLDER.getName()) || 				
				command.equals(MainActionCommands.OPEN_NEGATIVE_MODE_RAW_DATA_FOLDER.getName())) 
			openRecentDataFolder(command);
	
	}
	
	private void openRecentDataFolder(String command) {

		if (command.equals(MainActionCommands.OPEN_POSITIVE_MODE_RAW_DATA_FOLDER.getName())) {
			
			File posFolder = getPositiveDataFolder();
			if(posFolder != null && posFolder.exists()) {
				
				try {
					Desktop.getDesktop().open(posFolder);
		...
		if (command.equals(MainActionCommands.OPEN_NEGATIVE_MODE_RAW_DATA_FOLDER.getName())) {
			
			File negFolder = getNegativeDataFolder();
			if(negFolder != null && negFolder.exists()) {
				
				try {
					Desktop.getDesktop().open(negFolder);
				} catch (IOException e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				}
