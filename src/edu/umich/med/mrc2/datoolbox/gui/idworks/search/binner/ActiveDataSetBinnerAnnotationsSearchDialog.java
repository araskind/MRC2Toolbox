...
public class ActiveDataSetBinnerAnnotationsSearchDialog extends JDialog implements ActionListener, BackedByPreferences {

	/**
	 * 
	 */
	private static final long serialVersionUID = 6166594084842739656L;
	private static final Icon binnerLookupIcon = GuiUtils.getIcon("binnerLookupIcon", 32);
	private static final String PREFS_NODE = 
			"edu.umich.med.mrc2.datoolbox.ActiveDataSetBinnerAnnotationsSearchDialog";
	private Preferences preferences;
	private static final String BASE_DIRECTORY = "BASE_DIRECTORY";
	private BinnerAnnotationsImportPanel binnerAnnotationsImportPanel;
	private File baseDirectory;
	private MSMSClusteringParametersPanel msmsClusteringParametersPanel;
	
	public ActiveDataSetBinnerAnnotationsSearchDialog(ActionListener parent) {
		
		super();
		setModalityType(ModalityType.APPLICATION_MODAL);
		setResizable(true);
		setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);
		setTitle("Search active MSMS data set using Binner annotations");
		setIconImage(((ImageIcon)binnerLookupIcon).getImage());
		...
		binnerAnnotationsImportPanel = new BinnerAnnotationsImportPanel();
		getContentPane().add(binnerAnnotationsImportPanel, BorderLayout.CENTER);
	...
	public BinnerAnnotationLookupDataSet getDataSet() {
		return binnerAnnotationsImportPanel.getDataSet();
	}
	...
	public String getFeatureSetName() {
		return binnerAnnotationsImportPanel.getDataSetName();
	}
	...
	public Collection<String>validateParameters(){
		...
		if(getAllClusters().isEmpty())
			errors.add("No Binner annotation clusters in the data set.");
		
		String newSetName = getFeatureSetName();
		if(newSetName.isEmpty())
			errors.add("Please name the Binner annotations data set.");
		else {
			if(binnerAnnotationsImportPanel.getDataSet() == null) {

				BinnerAnnotationDataSetManager.refreshBinnerAnnotationLookupDataSetList();
				if(BinnerAnnotationDataSetManager.getBinnerAnnotationDataSetByName(newSetName) != null)
					errors.add("Binner annotations data set \"" + newSetName + "\" already exists.");
			}
		}
		return errors;
	}
	...
	public void loadPreferences(Preferences prefs) {
		...
		binnerAnnotationsImportPanel.setBaseDirectory(baseDirectory);
