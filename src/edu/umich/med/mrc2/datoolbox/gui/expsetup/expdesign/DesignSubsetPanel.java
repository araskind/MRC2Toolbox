...
import edu.umich.med.mrc2.datoolbox.gui.expsetup.expdesign.editor.ProjectDesignEditorPanel;
import edu.umich.med.mrc2.datoolbox.gui.expsetup.expdesign.stucturedit.ExpDesignEditorDialog;
...
public class DesignSubsetPanel extends DockableMRC2ToolboxPanel implements TableModelListener, ListSelectionListener {
			...
			if (selectedSubset.isLocked()) {
				...
				if(modificationCommand.equals(MainActionCommands.DELETE_DESIGN_SUBSET_COMMAND.getName())) {
					...
					if (delete == JOptionPane.YES_OPTION) {
						...
						currentExperiment.getExperimentDesign().removeDesignSubset(selectedSubset);
						switchDataPipeline(currentExperiment, activeDataPipeline);
	...
	private void saveDesignSubsetData() {
		...
		if(creatingNewSet){ //	New subset
			...
			currentExperiment.getExperimentDesign().addDesignSubset(newSubset);
			currentExperiment.getExperimentDesign().setActiveDesignSubset(newSubset);
			expDesignEditorDialog.setVisible(false);
			designSubsetTable.setModelFromProject(currentExperiment);
