...
public class DatabaseCompoundTable extends BasicTable {
	...
	private CompoundDatabaseTableModel model;
	...
	public DatabaseCompoundTable() {
		...
		model = new CompoundDatabaseTableModel();
		setModel(model);

		setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
		rowSorter = new TableRowSorter<CompoundDatabaseTableModel>(model);
		setRowSorter(rowSorter);

		rowSorter.setComparator(model.getColumnIndex(CompoundDatabaseTableModel.ID_COLUMN),
		...
		columnModel.getColumnById(CompoundDatabaseTableModel.ID_COLUMN)
			.setCellRenderer(msfIdRenderer);
		columnModel.getColumnById(CompoundDatabaseTableModel.MASS_COLUMN)
		...
		mma = new MouseMotionAdapter() {
			...
			public void mouseMoved(MouseEvent e) {
				...
				if(columnModel.isColumnVisible(columnModel.getColumnById(CompoundDatabaseTableModel.ID_COLUMN))) {

					if (columnAtPoint(p) == columnModel.getColumnIndex(CompoundDatabaseTableModel.ID_COLUMN))
	...
	public MsFeatureIdentity getSelectedIdentity() {
		...
		return (MsFeatureIdentity) model.getValueAt(
				...
				model.getColumnIndex(CompoundDatabaseTableModel.ID_COLUMN));
	}
	...
	public Collection<CompoundIdentity> getListedCompounds() {
		...
		int idCol = model.getColumnIndex(CompoundDatabaseTableModel.ID_COLUMN);
