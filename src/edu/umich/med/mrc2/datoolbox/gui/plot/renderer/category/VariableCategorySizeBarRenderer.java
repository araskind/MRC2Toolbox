...
//                    ? state.getVisibleSeriesCount() : dataset.getRowCount();
            ...
            int valueCount = 0;
            for(int rowNum =0; rowNum<dataset.getRowCount(); rowNum++) {
            	
            	for(int coNum =0; coNum<dataset.getColumnCount(); coNum++) {
                	
                	Number value = dataset.getValue(rowNum, coNum);
                	if(value != null && value.doubleValue() > 0)
                		valueCount++;
                }
            }           
            if (valueCount > 0) 
                state.setBarWidth(Math.min(used / valueCount, maxWidth));
            else 
     ...
     */
    protected double calculateBarW0(
    		CategoryPlot plot, 
