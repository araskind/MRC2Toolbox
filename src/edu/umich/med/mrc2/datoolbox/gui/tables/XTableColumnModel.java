...
public class XTableColumnModel extends DefaultTableColumnModel {
	 ...
	 *
	 */
	private static final long serialVersionUID = -1233814396019016036L;
	/**
	 ...
	 * TableColumn is not found in this column model or its visibility status did
	 * not change.
	 * <p>
	 *
	 * @param aColumn
	 *            the column to show/hide
	 * @param visible
	...
	public void setColumnVisible(TableColumn column, boolean visible) {
		...
		if (!visible) {
			...
			int visibleColumnCount = tableColumns.size();
			int allColumnCount = allTableColumns.size();
			int visibleIndex = 0;

			for (int completeIndex = 0; completeIndex < allColumnCount; completeIndex++) {

				TableColumn visibleColumn = (visibleIndex < visibleColumnCount ? (TableColumn) tableColumns.get(visibleIndex) : null);
				TableColumn testColumn = (TableColumn) allTableColumns.get(completeIndex);
	 ...
	 * uncommon. This method will always return the first visible or else the first
	 ...
	 *                if <code>column</code> is <code>null</code>
	 */
	 ...
	 *
	 * @param identifier
	 *            the identifier object to search for
	 * @param onlyVisible
	 ...
	 *
	 * @exception IllegalArgumentException
	 *                if <code>identifier</code> is <code>null</code>, or if no
	 ...
	 * @param onlyVisible
	 *            if set columnIndex is meant to be relative to all visible columns
	 *            only else it is the index in all columns
	 *
	...
	public TableColumn getColumn(int columnIndex, boolean onlyVisible) {
		...
		Vector columns = (onlyVisible ? tableColumns : allTableColumns);
		return (TableColumn) columns.elementAt(columnIndex);
	}
	...
	public TableColumn getColumnById(Object identifier) {
		...
		for (int columnIndex = 0; columnIndex < noColumns; ++columnIndex) {
			column = (TableColumn) allTableColumns.get(columnIndex);
