...
public class CompoundIdentityDatabaseLinkRenderer  extends DefaultTableCellRenderer 
	...
	public CompoundIdentityDatabaseLinkRenderer() {
		super();
	}
	...
	public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus,
		...
		String linkLabel = id.getPrimaryDatabaseId();
		if(!linkLabel.isEmpty()) {
			
			link =
				"<A HREF=\"" + id.getPrimaryLinkAddress() + "\">" + linkLabel + "</A>";
		}
	...
	public void mouseClicked(MouseEvent e) {
		...
		if(table.getCellRenderer(crow, ccol) instanceof CompoundIdentityDatabaseLinkRenderer) {
			...
			try {
				...
				CompoundIdentity id = null;
				if(cellValue instanceof CompoundIdentity)
					id = (CompoundIdentity)cellValue;
				
				if(cellValue instanceof MsFeatureIdentity)
					id = ((MsFeatureIdentity)cellValue).getCompoundIdentity();

				if(id != null)
					urlString = id.getPrimaryLinkAddress();
			} 
			catch (Exception e2) {
				//	e2.printStackTrace();
			}
