...
public class NewProjectDialog extends JDialog implements ActionListener {
	...
	public NewProjectDialog(ActionListener listener) {
		...
		try {
			experimentDirectoryLocationTextField.setText(Paths.get(baseDirectory.getCanonicalPath()).toString());
		} catch (IOException e) {
		...
		JButton btnSave = new JButton(MainActionCommands.NEW_PROJECT_COMMAND.getName());
		btnSave.setActionCommand(MainActionCommands.NEW_PROJECT_COMMAND.getName());
	...
	private void createNewProject() {
		...
		if(projLocation.isEmpty() || projName.isEmpty()) {
			MessageDialog.showErrorMsg("Project name and location must be defined", this);
			return;
		}
		...
		if(projFile.exists()) {
			MessageDialog.showErrorMsg(
					"Project \""+ projName + "\" already exists at \"" 
							+ projLocation + "\"", this);
			return;
		}	
		String pdesc = descriptionTextArea.getText().trim();
		if(activeExperiment != null)
			MRC2ToolBoxCore.getMainWindow().createNewProjectFromLimsExperiment(
