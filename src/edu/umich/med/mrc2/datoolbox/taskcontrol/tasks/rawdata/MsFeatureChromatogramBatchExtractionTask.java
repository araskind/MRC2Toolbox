...
public class MsFeatureChromatogramBatchExtractionTask extends AbstractTask implements TaskListener {
	...
	private Map<DataFile, Map<MSFeatureInfoBundle, ChromatogramDefinition>>featureChromatogramDefinitionsByFile;
	...
	private boolean extractOnlyFromParentFiles;
	...
	public MsFeatureChromatogramBatchExtractionTask(
			...
			boolean extractOnlyFromParentFiles) {
		super();
		this.rawDataFiles = rawDataFiles;
		this.features = features;
		this.commonDefinition = commonDefinition;
		this.xicTarget = xicTarget;
		this.extractOnlyFromParentFiles = extractOnlyFromParentFiles;
	...
	public void run() {
		...
		if(extractOnlyFromParentFiles)
			createFeatureChromatogramDefinitionsByFile();
		...
		for(DataFile df : rawDataFiles) {
			...
			MsFeatureChromatogramExtractionTask task = null;
			if(extractOnlyFromParentFiles) {
				
				Map<MSFeatureInfoBundle, ChromatogramDefinition> fileDefs = 
						featureChromatogramDefinitionsByFile.get(df);
				if(fileDefs == null)
					fileDefs = featureChromatogramDefinitions;
				
				task = new MsFeatureChromatogramExtractionTask(df, fileDefs);
			}
			else {
				task = new MsFeatureChromatogramExtractionTask(
						df, featureChromatogramDefinitions);
			}
	...
	private void createFeatureChromatogramDefinitionsByFile() {

		taskDescription = "Creating chromatogram definitions ... ";
		total = features.size();
		processed = 0;
		featureChromatogramDefinitionsByFile = 
				new TreeMap<DataFile,Map<MSFeatureInfoBundle, ChromatogramDefinition>>();
		
		for(DataFile df : rawDataFiles) {

			List<MSFeatureInfoBundle> fileFeatures = features.stream().
					filter(f -> f.getDataFile().equals(df)).
					collect(Collectors.toList());
			featureChromatogramDefinitionsByFile.put
				(df, createChromatogramDefinitionMap(fileFeatures));
		}
	}
	...
	private void createFeatureChromatogramDefinitions() {
		...
		total = features.size();
		processed = 0;
		featureChromatogramDefinitions = createChromatogramDefinitionMap(features);
...
//		}
	...
	private Map<MSFeatureInfoBundle, ChromatogramDefinition>createChromatogramDefinitionMap(
			Collection<MSFeatureInfoBundle>featureList){
		
		taskDescription = "Creating chromatogram definitions ... ";
		total = featureList.size();
		processed = 0;
		Map<MSFeatureInfoBundle, ChromatogramDefinition>cdm = 
				new HashMap<MSFeatureInfoBundle, ChromatogramDefinition>();
		for(MSFeatureInfoBundle mfb : featureList) {
