...
public class LibraryExportTask extends AbstractTask {

	private File outputLibraryFile, sourceLibraryFile;
	private CompoundLibrary currentLibrary;
	private Collection<LibraryMsFeature>targetSubset, exportTargetList;
	...
	private TreeSet<String> unmatchedAdducts = new TreeSet<String>();
	private Map<LibraryMsFeature,MsFeatureStatisticalSummary>statsMap;
	private boolean addAdductName;
	...
	public LibraryExportTask(
		...
		taskDescription = "Exporting merged library '" + currentLibrary.getLibraryName() + "' in " + libraryFormat.getName();
	}
	...
	public LibraryExportTask(
		...
		this.libraryFormat = libraryFormat;
		this.combineAdducts = combineAdducts;
		this.sourceLibraryFile = null;
		statsMap =  new HashMap<LibraryMsFeature,MsFeatureStatisticalSummary>();

		taskDescription = "Exporting library '" + currentLibrary.getLibraryName() + "' in " + libraryFormat.getName();
	}
	...
	public LibraryExportTask(
		...
		taskDescription = "Exporting MS feature set '" + currentLibrary.getLibraryName() + "' in " + libraryFormat.getName();
	}

	public LibraryExportTask(
			File selectedFile,
			MsLibraryFormat libraryFormat2,
			File libraryInputFile,
			CompoundLibrary currentLibrary2,
			boolean combineAdducts2) {

		super();

		this.outputLibraryFile = selectedFile;
		this.currentLibrary = currentLibrary2;
		this.libraryFormat = libraryFormat2;
		this.combineAdducts = combineAdducts2;
		this.sourceLibraryFile = libraryInputFile;
		this.targetSubset = null;
		this.featureSubset = null;
		statsMap =  new HashMap<LibraryMsFeature,MsFeatureStatisticalSummary>();
	...
	private void createTargetList() {
		...
		if(featureSubset != null){
			...
			Collection<LibraryMsFeature>convertedFeatures = convertFeaturesToTargets(featureSubset);
	...
	private void writeTextLibrary() throws IOException {
		...
		total = exportTargetList.size();
		processed = 0;
		final Writer writer = new BufferedWriter(new FileWriter(outputLibraryFile));
		...
		//	Write feature data
		for(LibraryMsFeature lf : exportTargetList){
